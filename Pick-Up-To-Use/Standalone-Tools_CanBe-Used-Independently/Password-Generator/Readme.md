...
```
  ...
  This also indicates that when you use the literal form of the `-counts` mode, such as `abcd:4`, while also using `--allow-char-repeat=false`, you might be expecting a result like `bdca`. However, if you mistakenly input `abcc:4`, you will get a result like `bcca`, and this outcome is consistent with the program's logic for `--allow-char-repeat=false`. Therefore, you must be careful when using the literal form of the `-counts` mode.
