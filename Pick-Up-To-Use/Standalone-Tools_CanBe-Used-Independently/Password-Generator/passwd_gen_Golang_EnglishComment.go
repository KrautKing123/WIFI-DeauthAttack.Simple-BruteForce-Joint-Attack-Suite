...
//The printProgressBar function is called within a for-select-case loop. It is invoked whenever a new result is produced or a timer ticks, but the progress bar is only actually printed to the screen conditionally
func printProgressBar(current int64, total float64, isFinal bool) {
	...
	} //// Conclude the progress bar display:
	// 1. Print a newline to move to the next line in the terminal.
	// 2. Use Stderr, consistent with all other progress messages, to keep Stdout pure for program output.
}